...
const Messages = () => {
  ...
  const handleClick = async (user) => {
    ...
    const isPresent = state.allMessages.filter(
      (message) =>
        (message.firstUserId === session.id &&
          message.secondUserId === user._id) ||
        (message.firstUserId === user._id &&
          message.secondUserId === session.id)
    );

    console.log("isPresent on message.js file", isPresent);
    if (!isPresent.length) {
      console.log(
        "session and userId on messages.js file",
        session.id,
        user._id
      );
      await dispatch(messageActions.CREATE_MESSAGE, {
    ...
    setMessageBox(true);
    setSingleUser(user);
    if (isPresent.length) setMessageId(isPresent[0]._id);
